...
param (
    ...
    # The the example format of such parameter: "@`Sitecore.Core = Standard-S0-250
    #                                             Sitecore.Exm.master = Standard-S0-250
    #                                             Sitecore.Experienceforms = Standard-S0-250`"
    # The first part is the name of the DACPAC file, the second part e.g. 'Standard-S0-250' has edition, size and max DB size values separated by dash
    [string]$DBSizesString
)

$deployDatabases = $true

$message = "DBSizesString is " + [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String($DBSizesString))
...
if ($deployDatabases) {
    .\DeployDatabases.ps1 -ResourcesDirectory $ResourcesDirectory -SqlServer:$SqlServer -SqlAdminUser:$SqlAdminUser -SqlAdminPassword:$SqlAdminPassword -EnableContainedDatabaseAuth -SkipStartingServer -SqlElasticPoolName $SqlElasticPoolName -DatabasesToDeploy $DatabasesToDeploy -DBSizesString $DBSizesString
